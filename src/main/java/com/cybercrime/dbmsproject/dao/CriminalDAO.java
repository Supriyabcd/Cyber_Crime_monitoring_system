...
//     /** 🔍 Get all criminals for a specific case */
// public List<Criminal> findByCaseId(int caseId) {
//     return jdbcTemplate.query(
//         "SELECT * FROM Criminal WHERE case_id = ?",
//         rowMapper,
//         caseId
//     );
// }
...
public class CriminalDAO {
    ...
    // ✅ RowMapper with description added
    private final RowMapper<Criminal> rowMapper = (rs, rowNum) -> {
        Criminal c = new Criminal();
        int id = rs.getInt("criminal_id");
        c.setCriminalId(rs.wasNull() ? null : id);
        ...
        c.setMobNo(rs.getString("mob_no"));
        c.setSocialHandle(rs.getString("social_handle"));
        c.setDescription(rs.getString("detailed_description")); // ✅ new field
        return c;
    };
    ...
    public int save(Criminal c) {
        ...
        jdbcTemplate.update(conn -> {
            PreparedStatement ps = conn.prepareStatement(
                "INSERT INTO Criminal(case_id, officer_id, fname, lname, dob, h_no, street, city, state, mob_no, social_handle, detailed_description) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                Statement.RETURN_GENERATED_KEYS
            );
            ps.setInt(1, c.getCaseId());
            ...
            ps.setString(10, c.getMobNo());
            ps.setString(11, c.getSocialHandle());
            ps.setString(12, c.getDescription()); // ✅ new column
            return ps;
        }, keyHolder);
    ...
    public int update(Criminal c) {
        return jdbcTemplate.update(
            "UPDATE Criminal SET fname=?, lname=?, dob=?, h_no=?, street=?, city=?, state=?, mob_no=?, social_handle=?, detailed_description=? " +
            "WHERE criminal_id=? AND case_id=? AND officer_id=?",
            c.getFname(),
            c.getLname(),
            c.getDob() != null ? Date.valueOf(c.getDob()) : null,
            ...
            c.getMobNo(),
            c.getSocialHandle(),
            c.getDescription(), // ✅ included in update
            c.getCriminalId(),
            c.getCaseId(),
            c.getOfficerId()
        );
